  ...
  return (
    <div className="relative">
      {showBanner && packages.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-blue-500 text-black shadow-lg rounded-t-lg transition-transform duration-300">
          <div className="flex flex-col">
            <div className="flex overflow-x-auto whitespace-nowrap py-2 p-2">
              {packages.map((packageData) => (
                <div key={packageData._id} className="inline-block w-60 mx-2 bg-white text-black rounded-lg p-4 shadow-lg">
                  <img 
                    src={packageData.pImage} 
                    alt={packageData.package_Title} 
                    className="w-full h-24 object-cover rounded-md mb-2" 
                  />
                  <h2 className="text-sm font-bold">{packageData.package_Title}</h2>
                  <p className="text-xs">{packageData.packageDes.slice(0, 80)}...</p>
